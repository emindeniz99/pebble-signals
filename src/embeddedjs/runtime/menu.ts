 ...
 */
export function Menu(props: MenuProps): Content {
	...
	const applyActive = (raw: number): number => {
	...
	const applyScroll = (hot: number): void => {
		if (n === 0) return; // no rows -> nothing to scroll (empty-list branch)
		let next = scroll;
		if (hot * rowHeight < next)
			next = hot * rowHeight; // row's top is above the window -> reveal it
		else if ((hot + 1) * rowHeight > next + height) next = (hot + 1) * rowHeight - height; // row's bottom is below -> reveal it
