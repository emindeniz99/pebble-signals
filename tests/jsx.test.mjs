...
let compProps = null;
const comp = jsx((p) => { compProps = p; return jsx(Label, { string: "x" }); }, { a: 1 });
...
let threw = "";
try { jsx(123, {}); } catch (e) { threw = e.message; }
...
const btn = jsx(Container, {
	onPressSelect: () => { pressed++; },		// returns undefined -> truthy consume
...
appendChild(par, undefined); appendChild(par, null);
appendChild(par, false); appendChild(par, true);
check("nullish/boolean children are skipped", par.contents.length === 0);
appendChild(par, "text");
appendChild(par, 42);
check("string/number children become Labels", par.contents.length === 2
	&& par.contents[0].string === "text" && par.contents[1].string === "42");
appendChild(par, [new Label(null, {}), new Label(null, {})]);
check("array child recurses", par.contents.length === 4);
let fnErr = "";
try { appendChild(par, () => 1); } catch (e) { fnErr = e.message; }
...
sandbox.Container.prototype.focus = function () { focused = this; };
