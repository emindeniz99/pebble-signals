...
- **Runtime → strict TypeScript — LARGELY DONE.** `signals/jsx-runtime/flow` are
  ...
  `pebble build`). B6 DONE: the runtime-API `.d.ts` half is auto-generated by
  `npm run build:types` (`tsc --declaration` → `runtime-types/*.d.ts`); only
  the Piu HOST-globals half (`src/tsx/globals.d.ts`) stays hand-maintained,
  because those are host types (sourced from `@moddable/typings`), not ours
  to emit.
