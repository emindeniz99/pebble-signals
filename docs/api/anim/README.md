[**pebble-signals**](../README.md)

***

[pebble-signals](../README.md) / anim

# anim

## Type Aliases

- [SeqStep](type-aliases/SeqStep.md)
- [SequenceOptions](type-aliases/SequenceOptions.md)
- [SpringOptions](type-aliases/SpringOptions.md)
- [TweenOptions](type-aliases/TweenOptions.md)

## Functions

- [useSequence](functions/useSequence.md)
- [useSpring](functions/useSpring.md)
- [useTween](functions/useTween.md)
- [withDelay](functions/withDelay.md)
- [withRepeat](functions/withRepeat.md)
- [yoyo](functions/yoyo.md)
