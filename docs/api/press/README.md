[**pebble-signals**](../README.md)

***

[pebble-signals](../README.md) / press

# press

## Type Aliases

- [MultiClickOptions](type-aliases/MultiClickOptions.md)
- [PressButton](type-aliases/PressButton.md)
- [PressHandlers](type-aliases/PressHandlers.md)
- [RepeatClickOptions](type-aliases/RepeatClickOptions.md)

## Functions

- [useLongPress](functions/useLongPress.md)
- [useMultiClick](functions/useMultiClick.md)
- [useRepeatClick](functions/useRepeatClick.md)
