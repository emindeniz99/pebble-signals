...
> auto-wraps bare reactive reads). Full story: [Core concepts](concepts.md)
> · [API parity](api-parity.md).

**Start here:** [Getting started](getting-started.md) (SDK setup → first
face) · [Tutorials](#tutorials) · [Use it in your project](#use-it-in-your-own-project-npm)
· [Examples gallery](examples.md) · [All docs](README.md) ·
...
transport, not a quiet app — [troubleshooting](debugging.md).) Any
other example works the same way: `pnpm run dev -- --app pulse` runs the
[flagship face](examples.md).
...
| `TREESHAKE_ALLOW` | *(empty)* | comma list of dynamic-import specifiers that are **HOST** modules, e.g. `TREESHAKE_ALLOW="piu/Timeline,qrcode"` — they stop defeating the scan, so pruning stays ON while every *other* unresolvable import still self-disables. **Prefer this over `TREESHAKE_FORCE=1`**, which switches the whole safety scan off (an unresolvable `app/` target in the same build then prunes away silently). The host *prefixes* `pebble/`, `embedded:` and `piu/` are built in and need no key; a bare name does, because a hand-written `manifest.base.json` could legitimately map one. |
| `CRASH_UI` | `1` | ship `render()`'s on-watch crash SCREEN. `CRASH_UI=0` drops it (errors still log + contain) to reclaim boot symbols/bytes. **`CRASH_UI=0` with `MINIFY=0` is a hard build error**: the removal is dead-code elimination and the DCE rides the minifier, so unminified the flag would only stop the screen from installing while `showCrash`'s bytes still ship (findings P10). Keep the minifier, or keep the crash screen. |
...
> forced each decision — is in [`docs/design-journey.md`](design-journey.md).
...
[`docs/api-parity.md`](api-parity.md); the generated API reference
(`pnpm run docs`) is in [`docs/api/`](api/).
...
Conformance is CHECKED, not claimed: `tests/conformance.test.mts` runs 30
fine-grained-reactivity laws against the runtime (25 match Solid — including
the per-subtree `<ErrorBoundary>` — and 5 intentional divergences: run-once
components, per-effect error isolation, no memo equality-short-circuit, the
contained JSX binding, and the default-boundary crash screen — see
[`docs/api-parity.md`](api-parity.md)). Glitch-freedom and nested-effect
ownership flipped to MATCH in the 2026-07 core round. Since 2026-07-31 the
reference column is **executed, not cited**: `solid-js` and
`@preact/signals-core` are devDependencies (Node-side only — never bundled into
the mod, so the 32KB arena is untouched) and 28 of the 30 laws replay the same
scenario against the real library, with every intentional divergence pinned by
an assertion plus a reason rather than skipped. React stays documented: its
test-utils need a renderer + a DOM.
...
[`docs/perf-battery.md`](perf-battery.md) is the performance & battery
...
impact. [`docs/postmortem-navreactive-stack.md`](postmortem-navreactive-stack.md)
