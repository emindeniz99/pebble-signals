...
| `breeze` | **Weather face (⚠️ DEVICE-GATED — does not yet boot on gabbro; see the header banner).** City, a Bitham hero temperature with its unit on the same baseline (`°` is silent in Bitham, proven in Gothic — gotcha 20), an accent rule, condition, live HH:MM. Fed by the device-proven `useConfig` channel: partial payloads MERGE, last-known weather survives a reboot, a malformed payload is ignored. Drive it with `tools/config-drive.py` | _gated: boot fails on gabbro, undiagnosed (logs transport dead this session)_ |
| `mono` | **Minimal-typography face (⚠️ DEVICE-GATED, unverified — shares breeze's gate).** `bold 49px Roboto` time on the minute tick, a daypart-coloured 2px rule on the HOUR tick, the date on the DAY tick — each line subscribes at the coarsest boundary that can change it, and all three listeners ride the ONE shared firmware timer. Two Styles, no sensors, no phone | _device capture pending_ |
| `fuel` | **Battery face (⚠️ DEVICE-GATED, unverified — shares breeze's gate).** HH:MM over a 270° battery dial on one `runtime/draw` Canvas: arc LENGTH and COLOUR both carry the state (green charging, red ≤20%, calm blue otherwise) and the centred percent shifts with it. `useBattery` is seeded at construction, so the first paint is the true charge, not a placeholder | _device capture pending_ |
