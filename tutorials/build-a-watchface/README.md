# Build a watchface in pebble-signals
...
emulator. You will learn the whole pebble-signals mental model along the way —
signals, JSX bindings, layout, and *fine-grained* updates (only what changed
repaints; no VDOM diff).

This is the pebble-signals counterpart to the official Alloy watchface tutorial.
Alloy gives you imperative Piu; pebble-signals gives you React/Solid-style JSX +
signals that compile down to the same Piu nodes.

## Prerequisites

- The pebble-signals repo checked out and building (see the top-level `README.md`
...
2. **[Part 2 — date & layout](part2-date-and-layout.md)** — add a second and
   third line, a `<Column>`, and `Style`s. You learn how pebble-signals lays out
...
3. **[Part 3 — a live complication](part3-live-complication.md)** — an
   hour-based greeting. You learn the payoff: pebble-signals skips same-value
...
pebble-signals — like Alloy — targets **emery (Pebble Time 2, 200×228 square)**
