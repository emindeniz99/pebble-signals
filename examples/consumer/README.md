# pebble-signals consumer example — a FULL, device-verified app

Dogfoods the packaging (`docs/packaging.md`): a standalone Pebble app project
that consumes pebble-signals as an **installed npm library** (the tarball `npm
...
| `tsconfig.check.json` | STRICT typecheck against `node_modules/pebble-signals` |
| `src/tsx/examples/main.tsx` | the app — device `runtime/*` imports + Piu JSX |
| `src/app.tsx` | typecheck-only demo of the `pebble-signals/*` exports-map style |
| `src/pkjs/index.js` | phone-side fetch proxy glue |

## Run it

From the pebble-signals root, `npm run test:consumer` packs a fresh tarball,
...
no pebble-signals-specific glue needed. This proves the general case: **any**
zero/small-dependency, XS-safe (no DOM, no Node builtins) npm package can be
used in app code the same way.

Bundling-wise, esbuild only externalizes `runtime/*` (the preloaded pebble-signals
...
- The scaffold is still copy-the-shape; `create-pebble-signals` (roadmapped) turns
  it into `npm create`.
