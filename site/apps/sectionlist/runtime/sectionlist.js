...
const ROW_COLOR = "#808080";
const ACTIVE_COLOR = "white";
const ACTIVE_FILL = "#1a4d4d";
...
export function SectionList(props) {
    ...
    const selectedFlat = () => {
        if (selected === undefined || rowCount === 0)
            return -1;
    ...
    let winStart = 0;
    const keepInView = (selFlat) => {
        if (selFlat < 0)
            return; // no selection -> leave the window where it is
        let next = winStart;
        if (selFlat < winStart)
            next = selFlat; // above the window -> reveal its top
        else if (selFlat >= winStart + rows)
            next = selFlat - rows + 1; // below -> reveal bottom
    ...
    if (selected !== undefined) {
        if (typeof selected === "function")
            effect(() => keepInView(selectedFlat()));
        else
            keepInView(selectedFlat());
    }
    ...
    const host = new Column(null, { width, height });
    for (let slot = 0; slot < rows; slot++) {
        ...
        const cell = new Label(null, { width, height: rowHeight });
        ...
        effect(() => {
            ...
            let i = 0;
            while (i + 1 < secs.length && start[i + 1] <= fi)
                i++;
            if (fi === start[i]) {
